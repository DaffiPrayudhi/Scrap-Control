                        ...
                        <form method="get" action="<?= site_url('admnsmt/dashboardsmt') ?>" class="form-container">
                            <div class="form-row">
                                <div class="form-group col-sm-2">
                                    ...
                                    <input type="date" class="form-control" id="start_date" style="font-size: 14px" name="start_date" 
                                        value="<?= esc($filters['start_date']) ?>">
                                </div>
                                <div class="form-group col-md-2">
                                    <label for="end_date">End Date</label>
                                    <input type="date" class="form-control" id="end_date" name="end_date" style="margin-right: 20px; font-size: 14px"
...
<script>
    ...
    $('#mesin').change(function() {
        ...
        tipeNgSelect.empty();
        tipeNgSelect.append('<option value="">All Data Tipe NG</option>');
        ...
        if (mesin) {
            $.ajax({
                ...
                success: function(response) {
                    $.each(response.tipe_ngs, function(index, value) {
                        tipeNgSelect.append('<option value="' + value.tipe_ng + '">' + value.tipe_ng + '</option>');
                    });

                    tipeNgSelect.val('<?= esc($filters["tipe_ng"]) ?>'); 
                },
            ...
            $.ajax({
                ...
                success: function(response) {
                    $.each(response.models, function(index, value) {
                        modelSelect.append('<option value="' + value.model + '">' + value.model + '</option>');
                    });

                    modelSelect.val('<?= esc($filters["model"]) ?>');
                },
                error: function(xhr, status, error) {
